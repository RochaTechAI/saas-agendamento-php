...
<div class="container-fluid">
    <div class="row">
        <div class="col-md-3 col-lg-2 sidebar px-3 d-flex flex-column justify-content-between">
            <div>
                ...
                <ul class="nav flex-column">
                    <li class="nav-item"><a class="nav-link active" href="index.php?acao=painel"><i class="bi bi-calendar-check me-2"></i> Agenda do Dia</a></li>
                    <li class="nav-item"><a class="nav-link" href="index.php" target="_blank"><i class="bi bi-box-arrow-up-right me-2"></i> Site Público</a></li>
                </ul>
            </div>
            <div class="pb-4">
                <a href="index.php?acao=logout" class="btn btn-outline-danger w-100 fw-bold border-0 text-start px-3 text-danger"><i class="bi bi-box-arrow-left me-2"></i> Sair</a>
            </div>
        </div>
        ...
        <div class="col-md-9 col-lg-10 p-5">
            ...
            <div class="card p-4">
                <?php if (empty($agendamentos)): ?>
                    <div class="text-center py-5 text-muted"><i class="bi bi-inbox fs-1"></i><h5 class="mt-3">Nenhum paciente agendado.</h5></div>
                <?php else: ?>
                            ...
                            <tbody>
                                <?php foreach ($agendamentos as $agendamento): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <div class="fw-bold"><?= htmlspecialchars($agendamento['paciente_nome']) ?></div>
                                            <div class="small text-muted"><i class="bi bi-envelope"></i> <?= htmlspecialchars($agendamento['paciente_email'] ?? 'Não informado') ?> | <i class="bi bi-telephone"></i> <?= htmlspecialchars($agendamento['paciente_telefone'] ?? '') ?></div>
                                        </td>
                                        ...
                                        <td>
                                            <?php if($agendamento['status'] === 'agendado'): ?>
                                                <a href="index.php?acao=atualizar_status&status=concluido&id=<?= $agendamento['id'] ?>&data=<?= $data_desejada ?>" class="btn btn-sm btn-success fw-bold"><i class="bi bi-check2"></i> Concluir</a>
                                                <a href="index.php?acao=atualizar_status&status=cancelado&id=<?= $agendamento['id'] ?>&data=<?= $data_desejada ?>" class="btn btn-sm btn-outline-danger fw-bold ms-1"><i class="bi bi-x-lg"></i> Cancelar</a>
                                            <?php else: ?>
