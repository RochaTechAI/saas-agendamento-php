...
<head>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Hospital floor grid — perspective tiles */
    .scene-floor {
        position: absolute;
        bottom: -5%;
        left: -20%; right: -20%;
    ...
    /* Floating medical CROSS — CSS shape */
    .cross {
        position: absolute;
    ...
    .hex-ring {
        ...
        animation: pulseRing ease-in-out infinite;
    }

    @keyframes pulseRing {
        0%, 100% { transform: scale(1)    rotate(0deg);   opacity: 0.5; }
        50%       { transform: scale(1.1)  rotate(20deg);  opacity: 0.2; }
    }
    ...
    .text-gradient {
        background: linear-gradient(135deg, #fff 0%, #00d4ff 40%, #0066ff 70%, #7c3aed 100%);
    ...
    .btn-search {
        ...
        gap: 7px;
        flex-shrink: 0;
        box-shadow: 0 6px 24px rgba(0,102,255,0.4), inset 0 1px 0 rgba(255,255,255,0.15);
        position: relative;
        overflow: hidden;
    }
    ...
    .btn-search:hover {
        box-shadow: 0 8px 32px rgba(0,102,255,0.55), inset 0 1px 0 rgba(255,255,255,0.2);
    ...
    .doctor-card:hover {
        transform: perspective(1200px) rotateX(2deg) rotateY(-4deg) translateZ(16px);
        border-color: rgba(0,212,255,0.22);
        box-shadow:
            0 0 0 1px rgba(0,212,255,0.12),
            0 0 50px rgba(0,212,255,0.07),
            0 32px 80px rgba(0,0,0,0.7),
            -8px 8px 32px rgba(0,102,255,0.1),
            inset 0 1px 0 rgba(255,255,255,0.06);
    }
    ...
    .card-glow-strip {
        height: 2px;
        background: linear-gradient(90deg,
            transparent 0%, rgba(0,212,255,0.0) 20%,
            rgba(0,212,255,0.5) 50%,
            rgba(0,212,255,0.0) 80%, transparent 100%);
    ...
    .doc-avatar {
        ...
        flex-shrink: 0;
        letter-spacing: -1px;
        background: linear-gradient(135deg, #0066ff 0%, #00a8cc 50%, #0044aa 100%);
        box-shadow:
    ...
    .modal-box {
        background: linear-gradient(145deg, rgba(4,14,28,0.98) 0%, rgba(2,8,20,0.98) 100%);
        border: 1px solid rgba(0,212,255,0.14);
        border-radius: 28px;
        width: 100%;
        max-width: 460px;
    ...
    .modal-box::before {
        ...
        display: block;
        height: 2px;
        background: linear-gradient(90deg, transparent 5%, var(--cyan) 40%, #0066ff 60%, transparent 95%);
        opacity: 0.6;
    }
    ...
    .btn-confirm {
        width: 100%;
        padding: 14px;
        background: linear-gradient(135deg, #0066ff 0%, #00a0dd 100%);
        color: #fff;
        border: none;
        border-radius: 14px;
        font-size: 0.95rem;
...
<div class="scene" aria-hidden="true">
    ...
    <div class="cross" style="width:40px;height:40px; top:12%; left:7%;  animation-duration:9s;  animation-delay:0s;">
        <div class="cross-inner"></div>
    </div>
    <div class="cross" style="width:28px;height:28px; top:55%; left:3%;  animation-duration:12s; animation-delay:2s;">
        <div class="cross-inner"></div>
    </div>
    <div class="cross" style="width:55px;height:55px; top:20%; right:4%; animation-duration:11s; animation-delay:1s;">
        <div class="cross-inner"></div>
    </div>
    <div class="cross" style="width:32px;height:32px; top:70%; right:6%; animation-duration:8s;  animation-delay:3s;">
        <div class="cross-inner"></div>
    </div>
    <div class="cross" style="width:22px;height:22px; top:40%; left:92%;  animation-duration:14s; animation-delay:0.5s;">
        <div class="cross-inner"></div>
    </div>
    ...
    <div class="ecg-bar">
        <svg viewBox="0 0 1200 70" preserveAspectRatio="none" fill="none">
            <path d="M0,35 L100,35 L120,35 L130,5 L140,65 L150,15 L160,50 L170,35 L300,35
                     L320,35 L330,5 L340,65 L350,15 L360,50 L370,35 L500,35
                     L520,35 L530,5 L540,65 L550,15 L560,50 L570,35 L700,35
                     L720,35 L730,5 L740,65 L750,15 L760,50 L770,35 L900,35
                     L920,35 L930,5 L940,65 L950,15 L960,50 L970,35 L1100,35
                     L1120,35 L1130,5 L1140,65 L1150,15 L1160,50 L1170,35 L1200,35"
                stroke="rgba(0,212,255,0.6)" stroke-width="1.5" vector-effect="non-scaling-stroke"/>
        </svg>
    </div>
</div>
...
<div id="app" class="page">
    ...
    <header class="hero">
        ...
        <form @submit.prevent="buscarHorarios" class="search-card">
            ...
            <button type="submit" class="btn-search" :disabled="carregando">
                ...
                <span>{{ carregando ? 'Buscando...' : 'Buscar Horários' }}</span>
            </button>
        </form>
    </header>
    ...
    <main class="content">
        ...
        <div class="alert alert-ok">
            ...
            <div>
                ...
                <small>O horário foi reservado com sucesso. Você receberá uma confirmação por e-mail.</small>
            </div>
        </div>
        <?php endif; ?>
        ...
        <div class="alert alert-err">
            ...
            <div>
                ...
                <small>Outro paciente acabou de reservar este horário. Por favor, selecione outro.</small>
            </div>
        </div>
        <?php endif; ?>
        ...
        <div v-else-if="buscou && medicos.length === 0" class="empty-state">
            ...
            <p>Não encontramos médicos disponíveis nesta data.<br>Experimente um dia útil diferente.</p>
        </div>

        <!-- Doctor Cards -->
        <div v-else>
            <div v-for="medico in medicos" :key="medico.id" class="doctor-card"
                 @mousemove="tilt3d($event)"
                ...
                <div class="card-slots">
                    ...
                    <div class="slots-grid">
                        <button
                            ...
                            :key="horario"
                            class="slot-btn"
                            @click="abrirModal(medico, horario)"
                        >{{ horario }}</button>
                    </div>
                </div>
            </div>
    ...
    <footer class="site-footer">
        &copy; 2026 MedSaaS &nbsp;&middot;&nbsp;
        <a href="index.php?acao=login">Acesso administrativo</a>
    </footer>
        ...
        <div class="modal-box">
            ...
            <div class="modal-body">
                <form method="POST" action="index.php?acao=agendar">
                    ...
                    <div class="m-field">
                        <label>E-mail</label>
                        <input type="email" name="paciente_email"    placeholder="seu@email.com" required>
                    </div>
                    <div class="m-field">
                        <label>WhatsApp / Telefone</label>
                        <input type="text"  name="paciente_telefone" placeholder="(11) 99999-9999" required>
                    </div>
...
createApp({
    ...
    methods: {
        ...
        /* Efeito tilt 3D no hover do card */
        tilt3d(e) {
